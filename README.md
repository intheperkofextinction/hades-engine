...
* 🚀 **End-to-End Pipeline Orchestrator (`main.py`):** Unified execution pipeline running data ingestion, Monte Carlo risk modeling, Black Swan stress testing, and compliance auditing in a single command.
...
| :--- | :--- | :--- |
| **Orchestrator** | Python AsyncIO | Unified automated execution pipeline (`main.py`) |
...
| **Testing Suite** | `pytest` | Unit tests validating core quantitative logic (VaR/CVaR, GBM simulation) |
...
├── data_stream/
│   └── live_stream.py              # Simulated real-time price feed
├── database/
...
│   └── seed_real_prices.py         # Real historical market data fetcher (yfinance) & DB seeder
├── ingestion/
...
```bash
git clone https://github.com/intheperkofextinction/hades-engine.git
cd hades-engine
python3 -m venv venv
...
## 🗓️ Automated Execution & Scheduling

`main.py` serves as the CLI orchestrator for a single end-to-end pipeline run. It does not manage its own scheduling — this is a deliberate separation of concerns: scheduling, retries, and failure alerting are infrastructure responsibilities, not application logic.

In a production cloud environment, this script would be triggered on a daily cron schedule post-market-close via GitHub Actions, a Kubernetes CronJob, or an orchestrator like Apache Airflow — rather than running as a long-lived process with an internal sleep loop.
